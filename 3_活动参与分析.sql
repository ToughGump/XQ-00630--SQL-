...
user_activity_status AS (
    SELECT 
        roleid,
        MAX(CASE WHEN activity_name = 'newbie_pack' THEN 1 ELSE 0 END) AS joined_newbie,  --标记是否参与“新手礼包”活动
        MAX(CASE WHEN activity_name = 'challenge_complete' THEN 1 ELSE 0 END) AS joined_challenge --标记是否参与“挑战任务”活动
    FROM activity
    WHERE 
