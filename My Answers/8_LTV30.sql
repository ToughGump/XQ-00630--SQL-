...
april_new_user_30d_payment AS (
    SELECT 
        ...
        ,SUM(CASE 
            WHEN payment.time BETWEEN april_new_users.register_date AND DATE_ADD(april_new_users.register_date, INTERVAL 30 DAY) --限定30天
                THEN payment.money 
            ELSE 0 
...
SELECT
    ...
    ,ROUND(SUM(30d_total_pay) / (SELECT COUNT(DISTINCT roleid) FROM april_new_users), 2) AS `LTV30`
